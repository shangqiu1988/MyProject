...
@implementation UXinDBModel
+(void)initialize
{
    if(self != [UXinDBModel self]){
        [self createTable];
    }
}
-(instancetype)init
...
{
    __block BOOL res=NO;
    UXinDBHelper *dbHelper=[UXinDBHelper shareInstance];
...
+(BOOL)createTable
{
    ...
    [dbHelper.dbQueue inTransaction:^(FMDatabase * _Nonnull db, BOOL * _Nonnull rollback) {
        ...
        NSString *sql=[NSString stringWithFormat:@"CREATE TABLE IF NOT EXISRS %@(%@);",tableName,columeAndType ];
...
{
    for (UXinDBModel *model in array) {
        if (![model isKindOfClass:[UXinDBModel class]]) {
            return NO;
        }
    }
    __block BOOL res = YES;
...
+ (NSArray *)findAll
{
    ...
    [dbHelper.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            UXinDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
